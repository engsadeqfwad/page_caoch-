  ...
  return (
    <footer className="relative overflow-hidden bg-dark-950">
      ...
      <div
        ...
        style={{ background: 'linear-gradient(90deg, transparent, #BA785C, transparent)' }}
      />

      {/* Decorative radial blur */}
      <div
        className="absolute bottom-0 right-0 w-96 h-96 opacity-15 pointer-events-none blur-3xl"
        style={{ background: 'radial-gradient(circle, #5A2135 0%, transparent 70%)' }}
      />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="py-16 md:py-20 grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8">
          ...
          <div className="md:col-span-1">
            ...
            {/* Social links & hidden Email Icon */}
            <div className="flex items-center gap-3">
              {socialLinks.map((social) => (
                <a
                  ...
                  style={{
                    background: 'rgba(63,20,37,0.6)',
                    border: '1px solid rgba(239,208,213,0.2)',
                  }}
                  onMouseEnter={(e) => {
                    (e.currentTarget as HTMLAnchorElement).style.background = 'rgba(186,120,92,0.3)';
                    (e.currentTarget as HTMLAnchorElement).style.borderColor = '#BA785C';
                  }}
                  onMouseLeave={(e) => {
                    (e.currentTarget as HTMLAnchorElement).style.background = 'rgba(63,20,37,0.6)';
                    (e.currentTarget as HTMLAnchorElement).style.borderColor = 'rgba(239,208,213,0.2)';
                  }}
                  title={social.label}
                >
                  {social.icon}
            ...
            <nav className="space-y-3">
              {navLinks.map((link) => (
                <button
                  ...
                  className="block text-sm text-taupe hover:text-white transition-colors duration-200 text-right font-medium"
                >
                  {link.label}
                </button>
              ))}
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <span>تواصل معنا عبر الإيميل</span>
              </a>
            </div>
          </div>
        </div>
        ...
        <div
          ...
          style={{ borderTop: '1px solid rgba(239,208,213,0.1)' }}
        >
          <p className="text-xs text-taupe/60 text-center md:text-right font-medium">
            © 2025 د/ك : حنان خالد - Women's Fitness Coach. جميع الحقوق محفوظة.
          </p>
          <p className="text-xs text-gold font-bold text-center">
            منظومة رقمية صُممت خصيصاً للمرأة ✦
          </p>
        </div>
      </div>
    </footer>
